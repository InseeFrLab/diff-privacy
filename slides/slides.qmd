...
## Qu'est-ce que la *Differential Privacy* (*DP*) ?

<br>

- But : protéger un jeu de données $D$ tout en récupérant une sortie $S$

. . .

- Mécanisme DP : rendre quasi **indistinguable** les sorties obtenues avec ou sans un individu $i$  

. . .

- Si un individu peut avoir un fort impact sur la sortie $S$ → on ajoute davantage de bruit
...
- **Mécanisme** : algorithme aléatoire $M : D \mapsto M(D)$, qui associe à un **dataset** en entrée $D$ une **variable aléatoire** de sortie $s = M(\mathcal{D})$. 
La propriété DP est associée à l'algorithme
...
[^1]: [@dwork_algorithmic_2013] The Algorithmic Foundations of Differential Privacy. Foundations and Trends in Theoretical Computer Science
...
$$
→ On considère la variable aléatoire $\mathcal{L}_{D, D'}(s)$ obtenue en tirant $s$ selon la loi de $M(D)$
...
::: {.columns}
::: {.column width="50%"}
![Densité de probabilité des sorties $M(D)$ et $M(D')$ avec un bruit d'écart type $\sigma = \frac{\sqrt{2}}{\ln(3)}$](images/courbe_densite_laplace.png)
:::

::: {.column width="50%"}
![Perte de confidentialité inférieur à $\varepsilon = \ln(3)$](images/courbe_risque_laplace.png)
:::
:::
...
::: {.columns}
::: {.column width="50%"}
![Densité de probabilité des sorties $M(D)$ et $M(D')$ avec un bruit d'écart type $\sigma = \frac{\sqrt{2}}{\ln(3)}$](images/courbe_densite_gauss.png)
:::
...
## Composer plusieurs fois un mécanisme DP ?
...
- Si un mécanisme $M$ vérifie une des formes de DP, alors $f(M)$ également → stable au **post-processing**
...
- Fixer un seuil (max du jeu de données) peut révéler … le max !

  → La sensibilité n'est pas protégé par la DP et peut être inféré à partir de quelques requêtes 

. . .

- A mitiger dans notre cadre

---

## Solution : borner les valeurs dans un intervalle donné
...
- D'autres solutions sont évoquées dans la littérature : **Propose Test Release**, **Sample and Aggregate**, ect
...
- On a un système d'équations **linéaires** en les inverses des variances qu'on résout matriciellement
...
$$
\begin{aligned}
\frac{1}{\sigma^2_{\text{estimation}, (V_1, V_2)}} &= \frac{1}{\sigma^2_{\text{requête}, (V_1, V_2)}} \\
\frac{1}{\sigma^2_{\text{estimation}, V_1}} &= \frac{1}{m_2 \sigma^2_{\text{requête}, (V_1, V_2)}} + \frac{1}{\sigma^2_{\text{requête}, V_1}} \\
\frac{1}{\sigma^2_{\text{estimation}, V_2}} &= \frac{1}{m_1 \sigma^2_{\text{requête}, (V_1, V_2)}} + \frac{1}{\sigma^2_{\text{requête}, V_2}} \\
\frac{1}{\sigma^2_{\text{estimation}, Total}} &= \frac{1}{m_2 \sigma^2_{\text{estimation}, V_2}} + \frac{1}{m_1 \sigma^2_{\text{requête}, V_1}} + \frac{1}{\sigma^2_{\text{requête}, Total}} 
\end{aligned}
$$
