...
- **Sensibilité** $\Delta f = \max_{D \sim D'} \|f(D) - f(D')\|$  → impact maximal d’un individu
  - Comptage : $\Delta = 1$ 
...
Alors, leur composition est : $\left(k\varepsilon, k\delta\right) \text{-DP}$
...
- Soient $k$ mécanismes $M_1, \dots, M_k$, $\varepsilon$-DP, et une partition **disjointe** des données $D_1, \dots, D_k$.  
La composition parallèle définie par : $M(D) = (M_1(D_1), \dots, M_k(D_k))$ satisfait alors $\varepsilon$-DP
...
- $\text{nb}_{\text{hommes}} = 20 \quad \text{Vs} \quad \text{nb}_{\text{hommes}} = 12+14+17$
...
$$
\mathbb{Var}[\hat{T}_{\text{comp}}] = \sum_{k=1}^{n} w_k^2 \sigma_k^2 = \frac{1}{\sum_{j=1}^{n} 1 / \sigma_j^2} 
$$
...
# Cas des totaux et des moyennes
...
$$
\mathbb{P}[M_E(D, u, \varepsilon) = s] \propto \exp\left(\frac{\varepsilon u(D, s)}{2 \Delta u}\right),
$$
